"use client";import { useState } from "react";export default function Home() {// Client state to track the active checkout nodeconst [selectedBook, setSelectedBook] = useState<string | null>(null);// Structural mapping for your publicationsconst books = [{ id: "before-you-accept", src: "/before-you-accept.jpg", alt: "Before You Accept" },{ id: "sars-guide", src: "/sars-audit-ready.jpg", alt: "SARS Audit-Ready" },{ id: "missing-link", src: "/missing-link.jpg", alt: "Missing Link" },{ id: "disability-tax", src: "/disability-tax.jpg", alt: "Disability Tax Benefits" },{ id: "uif-guide", src: "/uif-guide.jpg", alt: "UIF Survival Guide" },];return (  {/* UPPER GEOMETRY: Split Layout triggered by selection state */}     {/* ACTIVE CHECKOUT NODE (Mounts on selection) */}
    {selectedBook && (
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-8 relative z-10">
      {/* THE STRUCTURAL PATCH: Dynamically scales the logo based on active state */}
      <div
        className={`w-full mx-auto flex justify-center transition-all duration-500 ease-out ${
          selectedBook ? 'max-w-xs py-2' : 'max-w-lg md:max-w-xl py-8'
        }`}
      >
        <img
    ...
    {books.map((book) => (
      ...
      >
        <div className="w-full h-full bg-zinc-900 flex items-center justify-center overflow-hidden shadow-lg border border-black relative">
          ...
          />
          <span className="text-xs text-zinc-600 absolute text-center px-2 z-0">{book.alt}</span>
        </div>
      </div>
    ))}
  </div>
